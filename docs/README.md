# Documentation

> 中文版: [README.zh-CN.md](README.zh-CN.md)

This folder documents how to use, validate, and maintain the skill packs in this repository.

## For Users

- **Getting started**: [GETTING_STARTED.md](GETTING_STARTED.md)
- **Playbooks (recommended bundles)**: [PLAYBOOKS.md](PLAYBOOKS.md)
- **Using with Codex**: [USING_WITH_CODEX.md](USING_WITH_CODEX.md)
- **Using with Claude Code**: [USING_WITH_CLAUDE.md](USING_WITH_CLAUDE.md)
- **Skills catalog**: [SKILLS_CATALOG.md](SKILLS_CATALOG.md)
- **FAQ / troubleshooting**: [FAQ.md](FAQ.md)

## For Contributors

- **Conversion workflow**: [WORKFLOW.md](WORKFLOW.md)
- **Skill pack format (house style)**: [SKILL_PACK_FORMAT.md](SKILL_PACK_FORMAT.md)
- **Quality bar**: [QUALITY_BAR.md](QUALITY_BAR.md)
- **Skill optimization workflow**: [SKILL_OPTIMIZATION_WORKFLOW.md](SKILL_OPTIMIZATION_WORKFLOW.md)
- **QA & CI**: [QA_AND_CI.md](QA_AND_CI.md)
- **Maintenance**: [MAINTENANCE.md](MAINTENANCE.md)

## About the Project

- **Why this project exists**: [WHY_THIS_PROJECT.md](WHY_THIS_PROJECT.md)
- **Attribution & IP policy**: [ATTRIBUTION_AND_IP.md](ATTRIBUTION_AND_IP.md)
- **Requesting upstream permission (recommended)**: [REQUESTING_UPSTREAM_PERMISSION.md](REQUESTING_UPSTREAM_PERMISSION.md)
- **Releases**: [RELEASES.md](RELEASES.md)

## Reference

- **Audit report**: [AUDIT_REPORT.md](AUDIT_REPORT.md)
- **External references**: [EXTERNAL_REFERENCES.md](EXTERNAL_REFERENCES.md)
- **Press kit (copy/paste)**: [PRESS_KIT.md](PRESS_KIT.md)
- **Project memory**: [PROJECT_MEMORY.md](PROJECT_MEMORY.md)
